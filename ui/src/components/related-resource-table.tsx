...
import { RelatedResource, ResourceType } from '@/types/api'
...
export function RelatedResourcesTable(props: {
  ...
  const relatedColumns = useMemo(
    (): Column<RelatedResource>[] => [
      {
        header: 'Kind',
        accessor: (rs: RelatedResource) => rs.type,
        align: 'left',
        cell: (value: unknown) => (
      ...
      {
        header: 'Name',
        accessor: (rs: RelatedResource) => rs,
        cell: (value: unknown) => {
          const rs = value as RelatedResource
...
function RelatedResourceCell({ rs }: { rs: RelatedResource }) {
